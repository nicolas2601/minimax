...
<main class="bg-canvas min-h-screen">
  <div class="max-w-md mx-auto px-4 md:px-6 py-6 md:py-10 space-y-6">
    <header>
      ...
      <h1 class="font-waldenburg text-4xl md:text-5xl font-light text-ink mt-1">Editar cuenta</h1>
      {#if account}
        <p class="text-sm text-muted mt-2">Modificá los datos de <strong class="text-ink">{account.name}</strong></p>
      {/if}
    </header>
